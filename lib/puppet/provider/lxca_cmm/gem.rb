...
Puppet::Type.type(:lxca_cmm).provide(:gem, parent: Puppet::Provider::Lxca) do
  ...
  def self.instances
    instances = []
    cmms = Puppet::Provider::Lxca.discover_all_cmms
  ...
  def filter_by_chassis
    ...
    Puppet::Provider::Lxca.filter_cmms_by_chassis(chassis: (@resource[:chassis]).to_s).map do |cmm|
      cmm.instance_variables.each do |att|
        puts "#{att} - #{cmm.instance_variable_get att}"
      end
    end
  ...
  def filter_by_uuid
    ...
    Puppet::Provider::Lxca.filter_cmms_by_uuid([(@resource[:uuid]).to_s]).map do |cmm|
      cmm.instance_variables.each do |att|
        puts "#{att} - #{cmm.instance_variable_get att}"
      end
    end
