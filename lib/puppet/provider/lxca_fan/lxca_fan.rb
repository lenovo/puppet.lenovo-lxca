...
Puppet::Type.type(:lxca_fan).provide(:lxca_fan) do
  desc 'Fan provider for LXCA resource'
  ...
  def filter_by_chassis
    ...
    @client.discover_fans(chassis: (@resource[:chassis]).to_s).map do |fan|
      fan.instance_variables.each do |att|
        puts "#{att} - #{fan.instance_variable_get att}"
      end
    end
  ...
  def filter_by_uuid
    ...
    @client.fetch_fans([(@resource[:uuid]).to_s]).map do |fan|
      fan.instance_variables.each do |att|
        puts "#{att} - #{fan.instance_variable_get att}"
      end
    end
