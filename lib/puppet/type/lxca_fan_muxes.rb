...
Puppet::Type.newtype(:lxca_fan_muxes) do

  ensurable do
    
    newvalue(:discover_all) do
      Puppet.notice "Fetching all the LXCA fan_mux elements. Results displayed below\n"
      provider.discover_all
    end

    newvalue(:filter_by_chassis) do
      Puppet.notice "Fetching all the LXCA fan_mux elements filtered by chassis. Results displayed below\n"
      provider.filter_by_chassis
    end
    
    newvalue(:filter_by_uuid) do
      Puppet.notice "Fetching LXCA fan_mux filtered by UUID. Results displayed below\n"
  ...
  newparam(:name, :namevar => true) do
    desc "Name of the lxca fan_mux resource"
  end
