...
describe Puppet::Type.type(:lxca_config_profile).provider(:lxca_config_profile) do
  ...
  describe 'for ensurable filter_by_id' do
    it 'has id as a parameter' do
      expect { config_profile.provider.filter_by_id }.to raise_error(Puppet::Error, %r{Attribute id is mandatory for the ensurable filter_by_id})
    end
  end

  describe 'for ensurable delete_config_profile' do
    it 'has id as a parameter' do
      expect { config_profile.provider.delete_config_profile }.to raise_error(Puppet::Error, %r{Attribute id is mandatory when ensure is set to delete_config_profile})
    end
  end

  describe 'for ensurable rename_config_profile' do
    it 'has id as a parameter' do
      expect { config_profile.provider.rename_config_profile }.to raise_error(Puppet::Error, %r{Attribute id is mandatory for the ensurable rename_config_profile})
    end
  end

  describe 'for ensurable rename_config_profile' do
    it 'has profile_name as a parameter' do
      expect { config_profile_with_id.provider.rename_config_profile }.to raise_error(Puppet::Error, %r{Attribute profile_name is mandatory for the ensurable rename_config_profile})
    end
  end

  describe 'for ensurable activate_config_profile' do
    it 'has id as a parameter' do
      expect { config_profile.provider.activate_config_profile }.to raise_error(Puppet::Error, %r{Attribute id is mandatory for the ensurable activate_config_profile})
    end
  end

  describe 'for ensurable activate_config_profile' do
    it 'has endpoint_uuid as a parameter' do
      expect { config_profile_with_id.provider.activate_config_profile }.to raise_error(Puppet::Error, %r{Attribute endpoint_uuid is mandatory for the ensurable activate_config_profile})
    end
  end

  describe 'for ensurable activate_config_profile' do
    it 'has restart as a parameter' do
      expect { config_profile_activate1.provider.activate_config_profile }.to raise_error(Puppet::Error, %r{Attribute restart is mandatory for the ensurable activate_config_profile})
    end
  end

  describe 'for ensurable unassign_config_profile' do
    it 'has id as a parameter' do
      expect { config_profile.provider.unassign_config_profile }.to raise_error(Puppet::Error, %r{Attribute id is mandatory for the ensurable unassign_config_profile})
    end
  end

  describe 'for ensurable unassign_config_profile' do
    it 'has power_down as a parameter' do
      expect { config_profile_unassign1.provider.unassign_config_profile }.to raise_error(Puppet::Error, %r{Attribute power_down is mandatory for the ensurable unassign_config_profile})
    end
  end

  describe 'for ensurable unassign_config_profile' do
    it 'has reset_imm as a parameter' do
      expect { config_profile_unassign2.provider.unassign_config_profile }.to raise_error(Puppet::Error, %r{Attribute reset_imm is mandatory for the ensurable unassign_config_profile})
    end
  end

  describe 'for ensurable unassign_config_profile' do
    it 'has force as a parameter' do
      expect { config_profile_unassign3.provider.unassign_config_profile }.to raise_error(Puppet::Error, %r{Attribute force is mandatory for the ensurable unassign_config_profile})
    end
  end
