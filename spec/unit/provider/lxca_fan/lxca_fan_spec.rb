...
describe Puppet::Type.type(:lxca_fan).provider(:lxca_fan) do
  ...
  describe "for ensurable filter_by_chassis" do
    it "should have chassis as a parameter" do
      expect {fan.provider.filter_by_chassis}.to raise_error(Puppet::Error, /Attribute chassis is mandatory for the ensurable filter_by_chassis/)
    end
  end

  describe "for ensurable filter_by_uuid" do
    it "should have uuid as a parameter" do
      expect {fan.provider.filter_by_uuid}.to raise_error(Puppet::Error, /Attribute uuid is mandatory for the ensurable filter_by_uuid/)
    end
  end
