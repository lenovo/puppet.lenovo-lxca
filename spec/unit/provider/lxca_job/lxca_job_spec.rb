...
describe Puppet::Type.type(:lxca_job).provider(:lxca_job) do
  ...
  describe 'for ensurable filter_by_id' do
    it 'has id as a parameter' do
      expect { job.provider.filter_by_id }.to raise_error(Puppet::Error, %r{Attribute id is mandatory for the ensurable filter_by_id})
    end
  end

  describe 'for ensurable filter_by_uuid' do
    it 'has uuid as a parameter' do
      expect { job.provider.filter_by_uuid }.to raise_error(Puppet::Error, %r{Attribute uuid is mandatory for the ensurable filter_by_uuid})
    end
  end

  describe 'for ensurable filter_by_state' do
    it 'has state as a parameter' do
      expect { job.provider.filter_by_state }.to raise_error(Puppet::Error, %r{Attribute state is mandatory for the ensurable filter_by_state})
    end
  end

  describe 'for ensurable cancel_job' do
    it 'has id as a parameter' do
      expect { job.provider.cancel_job }.to raise_error(Puppet::Error, %r{Attribute id is mandatory when ensure is set to cancel_job})
    end
  end

  describe 'for ensurable delete_job' do
    it 'has id as a parameter' do
      expect { job.provider.delete_job }.to raise_error(Puppet::Error, %r{Attribute id is mandatory when ensure is set to delete_job})
    end
  end
