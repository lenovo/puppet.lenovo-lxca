...
describe Puppet::Type.type(:lxca_node).provider(:lxca_node) do
  ...
  describe 'for ensurable filter_by_chassis' do
    it 'has chassis as a parameter' do
      expect { node.provider.filter_by_chassis }.to raise_error(Puppet::Error, %r{Attribute chassis is mandatory for the ensurable filter_by_chassis})
    end
  end

  describe 'for ensurable filter_by_uuid' do
    it 'has uuid as a parameter' do
      expect { node.provider.filter_by_uuid }.to raise_error(Puppet::Error, %r{Attribute uuid is mandatory for the ensurable filter_by_uuid})
    end
  end

  describe 'for ensurable power_on' do
    it 'has uuid as a parameter' do
      expect { node.provider.power_on }.to raise_error(Puppet::Error, %r{Attribute uuid is mandatory when ensure is set to power_on})
    end
  end

  describe 'for ensurable power_off' do
    it 'has uuid as a parameter' do
      expect { node.provider.power_off }.to raise_error(Puppet::Error, %r{Attribute uuid is mandatory when ensure is set to power_off})
    end
  end

  describe 'for ensurable power_restart' do
    it 'has uuid as a parameter' do
      expect { node.provider.power_restart }.to raise_error(Puppet::Error, %r{Attribute uuid is mandatory when ensure is set to power_restart})
    end
  end

  describe 'for ensurable blink_led' do
    it 'has uuid as a parameter' do
      expect { node.provider.blink_led }.to raise_error(Puppet::Error, %r{Attribute uuid is mandatory when ensure is set to blink_led})
    end
  end

  describe 'for ensurable turn_on_led' do
    it 'has uuid as a parameter' do
      expect { node.provider.turn_on_led }.to raise_error(Puppet::Error, %r{Attribute uuid is mandatory when ensure is set to turn_on_led})
    end
  end

  describe 'for ensurable turn_off_led' do
    it 'has uuid as a parameter' do
      expect { node.provider.turn_off_led }.to raise_error(Puppet::Error, %r{Attribute uuid is mandatory when ensure is set to turn_off_led})
    end
  end
