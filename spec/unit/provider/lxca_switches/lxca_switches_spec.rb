...
describe Puppet::Type.type(:lxca_switches).provider(:lxca_switches) do
  ...
  describe 'for ensurable filter_by_chassis' do
    it 'has chassis as a parameter' do
      expect { switches.provider.filter_by_chassis }.to raise_error(Puppet::Error, %r{Attribute chassis is mandatory for the ensurable filter_by_chassis})
    end
  end

  describe 'for ensurable filter_by_uuid' do
    it 'has uuid as a parameter' do
      expect { switches.provider.filter_by_uuid }.to raise_error(Puppet::Error, %r{Attribute uuid is mandatory for the ensurable filter_by_uuid})
    end
  end
